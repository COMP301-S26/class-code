...
public class MyLinkedList<E> implements MyList<E>, Iterable<E>{
    ...
    private class MySuperSecretEncapsulatedIterator implements Iterator<E> {
        ...
        /**
         * Creates a new iterator that points to the head of the linked list.
         * 
     ...
     * @return true if the collection changed as a result of this removal, false otherwise.
     */
    public boolean remove(E elem){
        int idx = this.indexOf(elem); // where is elem?
        boolean exists = idx != -1;   // does elem exist?
        if (exists) this.remove(idx); // remove if it exists
